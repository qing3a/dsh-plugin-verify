...
## 四、分类表（category 必填其一，与场景目录一致）

| 分类 | 说明 |
|---|---|
| 管理基建 | 插件市场、管理入口、备份 |
| 界面与体验 | 侧栏工作台、对话导航、UI 渲染、输入增强 |
| 编码与开发 | 代码操作、git 集成、代码审查、CLI 适配、测试 |
| 自动化 | 事件驱动、定时循环、断线续跑 |
| 视觉 | 图片问答、OCR、UI 还原、视觉桥接 |
| 搜索与联网 | 网页搜索、抓取、联网访问 |
| 记忆与上下文 | 长期记忆、记忆主权、上下文优化 |
| 调试与诊断 | 事件审计、会话诊断、运行观测 |
| 远程与访问 | 局域网访问、反向代理、远程控制 |
| 成本与用量 | Token 消耗、账户余额、预算 |
| 通知与提醒 | 桌面通知、任务进度、系统托盘 |
| 终端与 TUI | 终端界面、命令行增强 |
| 安全与权限 | 密钥扫描、权限规则、恶意模式检测 |
| 模型与编排 | 子代理委派、模型切换/回退、桥接 |
| 导入与迁移 | 从其他工具迁移会话/记忆 |
| 乐子与皮肤 | 桌面宠物、小游戏、皮肤美化 |
